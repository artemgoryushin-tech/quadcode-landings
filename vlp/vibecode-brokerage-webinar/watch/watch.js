(() => {
  ...
  const CHAT_KEY = "quadcodeVibecodeBrokerageWebinarChat";
  const CAPTIONS_KEY = "quadcodeVibecodeBrokerageWebinarCaptions";
  const duration = Math.max(1, Number(config.durationSeconds) || 1575.32);
  ...
  const formatLocalSessionTime = (sessionDate) => {
    try {
      const localTime = new Intl.DateTimeFormat(navigator.language || "en-GB", {
        hour: "numeric",
        minute: "2-digit",
        timeZoneName: "short",
      }).format(sessionDate);
  ...
  const appendViewerMessage = (message, shouldScroll = true) => {
    ...
    time.textContent = new Intl.DateTimeFormat(navigator.language || "en-GB", {
      hour: "2-digit",
      minute: "2-digit",
  ...
  const ATTENDANCE_KEY = "quadcodeVibecodeBrokerageWebinarAttendance";
  ...
  const updateSession = ({ forceMediaSync = false } = {}) => {
    ...
    if (latestSessionState.state === "live") {
      ...
      presenceLabel.textContent = "watching";
      setChatAvailability(true);
      startButton.disabled = false;
      startButton.setAttribute("aria-label", "Join the live webinar");
      startTitle.textContent = "Join the webinar";
      startCountdown.hidden = true;
      startNote.textContent = `Live · ${formatTime(liveOffset)} elapsed`;
    ...
    if (latestSessionState.state === "upcoming") {
      ...
      setMode("upcoming", "Upcoming session");
      viewerCount.hidden = false;
      viewerCount.textContent = "104";
      presenceLabel.textContent = "waiting";
      ...
      startTitle.textContent = "Starting in";
      startCountdown.hidden = false;
      startCountdown.textContent = countdown;
      startNote.textContent = localStartTime;
      startButton.setAttribute(
        "aria-label",
        `The webinar starts in ${countdown}, at ${localStartTime}`,
      );
      seekToLivePoint(0);
    } else {
      stage.dataset.state = "closed";
      setMode("ended", "Session ended");
      viewerCount.hidden = true;
      presenceLabel.textContent = "session ended";
      const countdown = formatCountdown(latestSessionState.millisecondsUntilStart);
      const localStartTime = formatLocalSessionTime(latestSessionState.nextStart);
      startTitle.textContent = "Next session in";
      startCountdown.hidden = false;
      startCountdown.textContent = countdown;
      startNote.textContent = localStartTime;
      startButton.setAttribute(
        "aria-label",
        `The next session starts in ${countdown}, at ${localStartTime}`,
      );
    }
  ...
  async function joinLive({ muted = false, automatic = false } = {}) {
    ...
    muteToggle.setAttribute("aria-label", muted ? "Turn sound on" : "Mute");
  ...
  if (hasVideo) {
    ...
    startButton.disabled = true;
    startTitle.textContent = "Video unavailable";
    startCountdown.hidden = true;
    startNote.textContent = "Please try again later";
  }
  ...
  document.addEventListener("fullscreenchange", () => {
    ...
    fullscreenToggle.setAttribute(
      "aria-label",
      active ? "Exit fullscreen" : "Open fullscreen",
    );
    showControls();
  });
  ...
  chatForm.addEventListener("submit", async (event) => {
    ...
    if (!config.chatEndpoint) {
      chatStatus.textContent = "Your message is saved in this browser.";
    ...
    try {
      ...
      chatStatus.textContent = "Your message was sent to the Quadcode team.";
    } catch {
      chatStatus.textContent =
        "Your message is saved in the browser, but delivery could not be confirmed.";
