(() => {
  ...
  const SOURCE_FORM = "quadcode_vibecode_brokerage_webinar";
  const SOURCE_SITE = "Quadcode Brokerage Solutions";
  const WEBINAR_TITLE = "Can You Vibe Code a Brokerage?";
  ...
  const SESSION_TIME = "09:00 UTC (12:00 GMT+3)";
  ...
  const pageReference = (pageUrl) => {
    try {
      ...
      return "quadcode.com/vlp/vibecode-brokerage-webinar/";
    }
  };
  ...
  const buildPayload = (registration, context = {}) => {
    ...
    const notes = [
      ...
      sessionStart ? `Session date: ${sessionStart}` : "",
      `Webinar URL: ${accessUrl}`,
      about ? `About the attendee:\n${about}` : "",
    ...
    payload.set(
      "lang_by_browser",
      context.language || browserLanguage || "en-GB",
    );
