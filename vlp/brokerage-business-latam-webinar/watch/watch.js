(() => {
  ...
  const REGISTRATION_KEY = "quadcodeLatamWebinarRegistration";
  const CHAT_KEY = "quadcodeLatamWebinarChat";
  const CAPTIONS_KEY = "quadcodeLatamWebinarCaptions";
  const duration = Math.max(1, Number(config.durationSeconds) || 1343.4);
  ...
  previewSessionStart.setUTCHours(Number(config.sessionHourUtc) || 18, 0, 0, 0);
  ...
  const formatLocalSessionTime = (sessionDate) => {
    try {
      const localTime = new Intl.DateTimeFormat("es-419", {
        hour: "numeric",
        minute: "2-digit",
        timeZoneName: "short",
      }).format(sessionDate);
      return `${localTime} · hora local`;
    } catch {
  ...
  const appendTimedMessage = (message, animate = true) => {
    const article = document.createElement("article");
    article.className = "chat-message chat-message--timed";
    if (!animate) article.classList.add("chat-message--batch");
    ...
    location.textContent = message.location || "Asistente";
  ...
  const appendViewerMessage = (message, shouldScroll = true) => {
    ...
    time.textContent = new Intl.DateTimeFormat("es-419", {
      hour: "2-digit",
      minute: "2-digit",
  ...
  const syncTimedChat = (playbackTime) => {
    ...
    dueMessages.forEach((message) => {
      if (!existingMessages.has(message.id)) {
        appendTimedMessage(message, !isLargeBatch);
        changed = true;
      }
    });

    if (changed && shouldStick) scrollChatToLatest(isLargeBatch ? "auto" : "smooth");
  ...
  const setChatAvailability = (isLive) => {
    ...
    chatInput.placeholder = isLive
      ? "Pregunta al equipo de Quadcode…"
      : "El chat se activa durante la sesión";
  };
  ...
  const updateSession = ({ forceMediaSync = false } = {}) => {
    ...
    if (latestSessionState.state === "live") {
      ...
      setMode("live", "En directo");
      viewerCount.hidden = false;
      viewerCount.textContent = String(viewersFor(liveOffset));
      presenceLabel.textContent = "en vivo";
      setChatAvailability(true);
      startButton.disabled = false;
      startButton.setAttribute("aria-label", "Unirme al webinar en directo");
      startTitle.textContent = "Unirme al webinar";
      startCountdown.hidden = true;
      startNote.textContent = `En directo · ${formatTime(liveOffset)} transcurridos`;
    ...
    if (latestSessionState.state === "upcoming") {
      ...
      setMode("upcoming", "Próxima sesión");
      viewerCount.hidden = false;
      viewerCount.textContent = "104";
      presenceLabel.textContent = "esperando";
      ...
      startTitle.textContent = "Comenzamos en";
      startCountdown.hidden = false;
      startCountdown.textContent = countdown;
      startNote.textContent = localStartTime;
      startButton.setAttribute(
        "aria-label",
        `El webinar comienza en ${countdown}, a las ${localStartTime}`,
      );
      seekToLivePoint(0);
    } else {
      stage.dataset.state = "closed";
      setMode("ended", "Sesión finalizada");
      viewerCount.hidden = true;
      presenceLabel.textContent = "sesión finalizada";
      const countdown = formatCountdown(latestSessionState.millisecondsUntilStart);
      const localStartTime = formatLocalSessionTime(latestSessionState.nextStart);
      startTitle.textContent = "Próxima sesión en";
      startCountdown.hidden = false;
      startCountdown.textContent = countdown;
      startNote.textContent = localStartTime;
      startButton.setAttribute(
        "aria-label",
        `La próxima sesión comienza en ${countdown}, a las ${localStartTime}`,
      );
    }
  ...
  const setCaptions = (enabled) => {
    ...
    captionsToggle.setAttribute(
      "aria-label",
      captionsEnabled ? "Desactivar subtítulos" : "Activar subtítulos",
    );
  ...
  if (hasVideo) {
    ...
    startButton.disabled = true;
    startTitle.textContent = "Video no disponible";
    startCountdown.hidden = true;
    startNote.textContent = "Vuelve a intentarlo más tarde";
  }
  ...
  chatForm.addEventListener("submit", async (event) => {
    ...
    if (!config.chatEndpoint) {
      chatStatus.textContent = "Mensaje guardado en este navegador.";
    ...
    try {
      ...
      chatStatus.textContent = "Mensaje enviado al equipo de Quadcode.";
    } catch {
      chatStatus.textContent =
        "El mensaje se guardó localmente; no se pudo confirmar la entrega.";
