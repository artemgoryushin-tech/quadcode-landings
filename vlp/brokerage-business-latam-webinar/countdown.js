(() => {
  ...
  const formatSessionLabel = (sessionInput = getNextSession()) => {
    try {
      ...
      return `${localTime} · tu hora local`;
    } catch {
  ...
  const initCountdown = (root) => {
    ...
    const update = () => {
      ...
      heading.textContent = isStarting
        ? "El webinar está comenzando"
        : "El próximo webinar comienza en";
      ...
      clock.setAttribute(
        "aria-label",
        isStarting
          ? `El webinar comienza a las ${formatSessionLabel(target)}`
          : `${remaining.hours} horas, ${remaining.minutes} minutos y ${remaining.seconds} segundos para el próximo webinar`,
      );
    };
