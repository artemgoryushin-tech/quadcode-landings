(() => {
  ...
  const SOURCE_FORM = "quadcode_philippines_webinar";
  const SOURCE_SITE = "Quadcode Brokerage Solutions";
  const WEBINAR_TITLE = "Brokerage Business in the Philippines: How to Start?";
  const SESSION_TIME = "12:00 GMT+3";
  ...
  const pageReference = (pageUrl) => {
    try {
      ...
      return "quadcode.com/vlp/brokerage-business-philippines-webinar/";
    }
  };
  ...
  const buildPayload = (registration, context = {}) => {
    ...
    const notes = [
      ...
      `Requested stage: registered webinar (${statusId})`,
      `Session time: ${SESSION_TIME}`,
      `Webinar URL: ${webinarUrl}`,
      about ? `Tell about yourself:\n${about}` : "",
      whyJoin ? `Why would you like to join:\n${whyJoin}` : "",
    ...
    payload.set("lang_by_browser", context.language || browserLanguage || "en");
      ...
      if (rejected) {
        throw new Error(
          responseMessage(responseBody) ||
            "Could not register you for the webinar. Please try again.",
        );
      }
