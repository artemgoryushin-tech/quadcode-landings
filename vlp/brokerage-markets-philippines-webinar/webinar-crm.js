(() => {
  ...
  const SOURCE_FORM = "quadcode_philippines_markets_webinar";
  const SOURCE_SITE = "Quadcode Brokerage Solutions";
  const WEBINAR_TITLE = "Best Markets to Launch a Brokerage in 2026";
  ...
  const pageReference = (pageUrl) => {
    try {
      ...
      return "quadcode.com/vlp/brokerage-markets-philippines-webinar/";
    }
  };
  ...
  const buildPayload = (registration, context = {}) => {
    ...
    payload.set(
      "lang_by_browser",
      context.language || browserLanguage || "en-PH",
    );
