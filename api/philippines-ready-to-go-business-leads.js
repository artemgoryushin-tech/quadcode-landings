...
function landingReference(sourceUrl, pagePath) {
  ...
  return pagePath || "Quadcode Philippines ready-to-go brokerage business landing";
}
...
export default async function handler(request, response) {
  ...
  const language = readString(body, "lang_by_browser", 20) || "tl";
  ...
  const context = [
    `${SOURCE_SITE} lead`,
    "Request type: Philippines Ready-to-Go Brokerage Business Demo",
  ...
  try {
    ...
    if (!crmResponse.ok || (result && typeof result === "object" && result.success === false)) {
      console.error("CRM rejected Philippines ready-to-go demo request", { status: crmResponse.status, body: responseText.slice(0, 500) });
