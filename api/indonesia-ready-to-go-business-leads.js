...
function landingReference(sourceUrl, pagePath) {
  ...
  return pagePath || "Quadcode Indonesia ready-to-go brokerage business landing";
}
...
export default async function handler(request, response) {
  ...
  const language = readString(body, "lang_by_browser", 20) || "id";
  ...
  const context = [
    `${SOURCE_SITE} lead`,
    "Request type: Indonesia Ready-to-Go Brokerage Business Demo",
  ...
  try {
    ...
    if (!crmResponse.ok || (result && typeof result === "object" && result.success === false)) {
      console.error("CRM rejected Indonesia ready-to-go demo request", { status: crmResponse.status, body: responseText.slice(0, 500) });
