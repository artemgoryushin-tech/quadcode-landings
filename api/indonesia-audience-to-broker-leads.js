...
function landingReference(sourceUrl, pagePath) {
  if (sourceUrl) {
    try {
      ...
      // Fall back to the submitted path.
    }
  }
  return pagePath || "Quadcode Indonesia audience-to-broker landing";
}
...
module.exports = async function handler(request, response) {
  ...
  const context = [
    `${SOURCE_SITE} lead`,
    "Request type: Indonesia Brokerage Demo Request",
  ...
  try {
    ...
    if (!crmResponse.ok || (result && typeof result === "object" && result.success === false)) {
      console.error("CRM rejected Indonesia demo request", { status: crmResponse.status, body: responseText.slice(0, 500) });
